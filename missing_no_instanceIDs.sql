WITH parameters AS (
    SELECT
        '2020-07-01'::date AS start_date,
        '2022-06-30'::date AS end_date,
	/*Enter item status*/
        'Missing'::varchar AS item_status_filter, --  Examples: Missing
              ---- Fill out one location or service point filter, leave others blank ----
        'Law'::varchar AS item_permanent_location_filter, -- Examples: Olin, ILR, Africana, etc.
...
FROM
    ...
    LEFT JOIN folio_reporting.holdings_ext AS he ON its.holdings_record_id = he.holdings_id
    LEFT JOIN public.inventory_instances as ins on he.instance_id=ins.id
    LEFT JOIN folio_reporting.instance_ext AS ie ON he.instance_id = ie.instance_id
